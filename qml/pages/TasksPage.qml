...
Page {
    ...
    Button {
        ...
        width: buttons.width / 7
        anchors.topMargin: buttons.height * 0.5
        anchors.leftMargin: 10
        anchors.left: parent.left
        onClicked: {
            urgency.visible = !urgency.visible;
            due.visible = !due.visible;
        }
    }

    function sortModel(comparator) {
        var tasks = []
        for (var i = 0; i < listModel.count; i++) {
            tasks.push(JSON.parse(JSON.stringify(listModel.get(i))));
        }
    ...
    Row {
        ...
        Button {
            width: parent.width / 3
            text: "all"
            onClicked: { currentMode = allTasksMode; showTasks(); }
        }
        Button {
            width: parent.width / 3
            text: "completed"
